 ...
 * 
 */
public class TestServerComplexDefaultHandleListener implements HandleListener
{
	private List<String> testData = new ArrayList<String>();
	private final Logger log = LoggerFactory
			.getLogger(TestServerComplexDefaultHandleListener.class);
	public TestServerComplexDefaultHandleListener()
	{
		for (int i = 0; i < 500; i++)
		{
	...
	public void read(MessageEvent event)
	{
		// TODO Auto-generated method stub
		// 判断确定类型后执行
		System.out.println("read:"+event.getMessage());
		/*if (testData.size() > 0)
		{
